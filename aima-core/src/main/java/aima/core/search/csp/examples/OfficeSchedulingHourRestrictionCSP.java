...
public class OfficeSchedulingHourRestrictionCSP extends CSP<Variable, List<Integer>> {

	public OfficeSchedulingHourRestrictionCSP(int hourRestrictionStart, int hourRestrictionEnd) {
		...
		for (int hour : horarios_A) {
			if(hour >= hourRestrictionStart && hour <= hourRestrictionEnd) {
				temp.add(hour);
			}
		}
		horarios_A = temp.stream()
		...
		temp = new ArrayList<>();
		for (int hour : horarios_B) {
			if(hour >= hourRestrictionStart && hour <= hourRestrictionEnd) {
				temp.add(hour);
			}
		}
		horarios_B = temp.stream()
	        .mapToInt(Integer::intValue)
	        .toArray();
		...
		temp = new ArrayList<>();
		for (int hour : horarios_C) {
			if(hour >= hourRestrictionStart && hour <= hourRestrictionEnd) {
				temp.add(hour);
			}
		}
		horarios_C = temp.stream()
	        .mapToInt(Integer::intValue)
	        .toArray();
		...
		temp = new ArrayList<>();
		for (int hour : horarios_D) {
			if(hour >= hourRestrictionStart && hour <= hourRestrictionEnd) {
				temp.add(hour);
			}
		}
		horarios_D = temp.stream()
	        .mapToInt(Integer::intValue)
	        .toArray();
		...
		temp = new ArrayList<>();
		for (int hour : horarios_E) {
			if(hour >= hourRestrictionStart && hour <= hourRestrictionEnd) {
				temp.add(hour);
			}
		}
		horarios_E = temp.stream()
	        .mapToInt(Integer::intValue)
	        .toArray();
