...
public class OfficeSchedulingPriorityNewNormalityCSP extends CSP<Variable, List<Integer>> {

	public OfficeSchedulingPriorityNewNormalityCSP() {
		addVariable(new Variable("Alice (vaccinated)"));
		addVariable(new Variable("Bob"));
		addVariable(new Variable("Charlie"));
		addVariable(new Variable("David (vaccinated)"));
		addVariable(new Variable("Eve (vaccinated)"));
		...
		for (int i = 0; i < 5; i++) {
			...
			Boolean isVar1Vaccinated = var1.getName().contains("vaccinated");

			for (int j = i + 1; j < 5; j++) {
				Variable var2 = getVariables().get(j);
				Boolean isVar2Vaccinated = var2.getName().contains("vaccinated");

				if(!isVar1Vaccinated || !isVar2Vaccinated) {
