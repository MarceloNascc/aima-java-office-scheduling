...
public class OfficeSchedulingPriorityWorkingCSP extends CSP<Variable, List<Integer>> {

	public OfficeSchedulingPriorityWorkingCSP() {
		addVariable(new Variable("Alice (Dep. Charlie)"));
		addVariable(new Variable("Bob"));
		addVariable(new Variable("Charlie"));
		addVariable(new Variable("David (Dep. Charlie)"));
		addVariable(new Variable("Eve (Dep. Charlie)"));
		...
		// David have to wait until Charlie finishes his work
		addConstraint(new StartAfterConstraint(david, charlie));

		// Eve have to wait until Charlie finishes his work
		addConstraint(new StartAfterConstraint(eve, charlie));
		
		// Alice have to wait until Charlie finishes his work
		addConstraint(new StartAfterConstraint(alice, charlie));
	}
}